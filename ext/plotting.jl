...
function markers_from_keys(ukeys)
    MARKERS = [
        :circle, :dtriangle, :rect, :star5, :xcross, :diamond,
...
function Attractors.heatmap_basins_attractors!(ax, BoA::ArrayBasinsOfAttraction; kwargs...)
    return Attractors.heatmap_basins_attractors!(ax, BoA.grid, BoA.basins, BoA.attractors; kwargs...)
end
...
# Shaded basins
##########################################################################################
function Attractors.shaded_basins_heatmap(
        grid, basins::AbstractArray, attractors, iterations;
...
function Attractors.shaded_basins_heatmap(
        BoA::ArrayBasinsOfAttraction, iterations;
        show_attractors = true,
        maxit = maximum(iterations),
        kwargs...
    )
    return Attractors.shaded_basins_heatmap(
        BoA.grid, BoA.basins, BoA.attractors, iterations;
        show_attractors = show_attractors, maxit = maxit, kwargs...
    )
end

function Attractors.shaded_basins_heatmap!(
        ax, grid, basins, iterations, attractors;
        ukeys = unique(basins),
        show_attractors = true,
        maxit = maximum(iterations)
    )
    ...
    ids = 1:length(ukeys)
    replace_dict = Dict(k => i for (i, k) in enumerate(ukeys))
    basins_to_plot = replace(basins .* 1.0, replace_dict...)
    access = SVector(1, 2)
...
function Attractors.shaded_basins_heatmap!(
        ax, BoA::ArrayBasinsOfAttraction, iterations;
        ukeys = unique(basins), show_attractors = true, maxit = maximum(iterations)
    )
    return Attractors.shaded_basins_heatmap!(
        ax, BoA.grid, BoA.basins, iterations, BoA.attractors;
...
function Attractors.plot_basins_curves!(
...
function Attractors.plot_attractors_curves!(
        ax, attractors_cont, attractor_to_real, prange = 1:length(attractors_cont);
        kwargs...
    )
    ...
    return plot_continuation_curves!(ax, continuation_info, prange; kwargs...)
end

function Attractors.plot_continuation_curves!(
...
function Attractors.plot_basins_attractors_curves(
        fractions_cont, attractors_cont, a2r::Function, prange = 1:length(attractors_cont);
        kwargs...
    )
...
function Attractors.plot_basins_attractors_curves!(
        axb, axa, fractions_cont, attractors_cont,
    ...
    plot_attractors_curves!(
        axa, attractors_cont, attractor_to_real, prange;
