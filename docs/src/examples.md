...
## Enriching a continuation with more measures of nonlocal stability

In this example we will perform a continuation in a modified Lorenz-like system that goes through a bistability of a chaotic attractor and a limit cycle.
We will enrich this continuation with more measures of nonlocal stability (Lyapunov exponents, minimal fatal shock), utilizing on one hand the power of **DynamicalSystems.jl** that provides out of the box several algorithms for dynamical systems, and on the other hand the visualization convenience function [`plot_continuation_curves`](@ref).

First, let's define the system
```@example MAIN
...
p0 = [5.0, 0.1]
u0s = [[4.0, 5, 0], [-4.0, -5, 0]]
ds = CoupledODEs(modified_lorenz_rule, u0s[1], p0;
    diffeq = (alg = Vern9(), abstol = 1e-9, reltol = 1e-9, dt = 0.01)
)
```

and now find and plot its attractors
```@example MAIN
grid = (
    range(-10.0, 10.0; length = 100),
    range(-15.0, 15.0; length = 100),
    range(-15.0, 15.0; length = 100),
)
...
for (k, A) in attractors
    scatter!(ax, A[:, 1], A[:, 2]; label = string(fs[k]))
end
axislegend(ax)
fig
```

We see that for the chosen parameters there is a coexistence of a limit cycle and a chaotic attractor. We can confirm that both attractors are robust as both have sufficiently large basin fractions.

Let's now perform a continuation by re-using the `mapper` we created above.
But first, as we plan to enrich this continuation, lets setup more axes than the default two that come out of the box with the visualization functions:
```@example MAIN
figcont = Figure(size = (600, 600))
axs = [Axis(figcont[i, 1]) for i in 1:4]
axs[1].ylabel = "basins %"
axs[2].ylabel = "attractor min(x)"
axs[3].ylabel = "Lyapunov"
axs[4].ylabel = "MFS"
axs[4].xlabel = "parameter"
figcont
```

The first component is to do a continuation like we have done in many other examples in the library
...
plot_basins_attractors_curves!(axs[1], axs[2],
...
lis = map(enumerate(prange)) do (i, p) # loop over parameters
    ...
    Dict(k => first(lyapunovspectrum(ds, 10_000; u0 = A[1])) for (k, A) in attractors)
end
```
...
figcont
```
This reveals a plethora of information for tha attractors that we would otherwise obtain by visualizing the system dynamics at every single parameter: the dynamics start with a limit cycle (0 Lyapunov exponent), go into bi-stability of chaos and limit cycle, then there is only one limit cycle again, and then a chaotic attractor appears again.

The last piece of information to add is yet another measure of nonlocal stability: the minimal fatal shock. For that, we once more re-use the created `mapper`, but now we must not forget to reset it inbetween parameter increments:

```@example MAIN
# input arguments to the MFS estimation
using LinearAlgebra: norm
search_area = collect(extrema.(grid ./ 2))
search_algorithm = MFSBlackBoxOptim(max_steps = 5000, guess = ones(3))
...
mfss = map(enumerate(prange)) do (i, p)
    ...
    # otherwise, compute the actual MFS
    Dict(k =>
...
and we add this to the continuation plot as well

```@example MAIN
plot_continuation_curves!(axs[4], mfss, prange; add_legend = false)
figcont
```