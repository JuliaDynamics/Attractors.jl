...
function silhouettes_new(dbscanresult::Clustering.DbscanResult, dists::AbstractMatrix)
    ...
    clusters = [findall(x -> x == i, labels) for i in 1:maximum(labels)] #all clusters
    if length(clusters) == 1
        return zeros(length(clusters[1]))
    end #all points in the same cluster -> sil = 0
...
function _calcb!(i, idx_c_i, dists, clusters, outsideclusters)
    ...
    for (idx_c, cluster) in enumerate(clusters)
        ...
        if dist_to_clstr < min_dist_to_clstr
            min_dist_to_clstr = dist_to_clstr
        end
    end
...
#####################################################################################
function optimal_radius_dbscan(
        features, min_neighbors, metric, optimal_radius_method,
...
"""
function optimal_radius_dbscan_silhouette(
        features, min_neighbors, metric,
        num_attempts_radius, silhouette_statistic
    )
...
function optimal_radius_dbscan_knee(_features::Vector, min_neighbors, metric)
    ...
    d, n = size(features)
    features_vec = [features[:, j] for j in 1:n]
