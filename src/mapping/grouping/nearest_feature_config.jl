...
[`AttractorsViaFeaturizing`](@ref) are always labelled according to the feature
from the `tempates` that is closest.
...
function group_features(features::Vector{<:AbstractVector}, config::GroupViaNearestFeature)
   ...
   labels_nested, dists_to_templates = bulksearch(template_tree, features, NeighborNumber(1))
   labels = reduce(vcat, labels_nested) # make it a vector
   distances = reduce(vcat, dists_to_templates)
