...
@testset "Newton 2d" begin
    ...
    for i in (keys(randomised))
        if norm(randomised[i]) >= 0.65 || norm(randomised[i]) <= 0.6 || newton(randomised[i] + i) == newton(i)
            test = false
        end
    end
    @test test
    ...
    @testset "target id" begin
        ...
        for mfs in (mfs_1, mfs_2)
            for f in mfs
                @test norm(f[2]) ≈ 0.622 rtol = 1.0e-2
            end
        end
    end
end
...
@testset "Magnetic 2D" begin
    ...
    @test all(x -> isapprox(x, 0.395; atol = 1.0e-2), randomised_r)
    ...
    @test all(x -> isapprox(x, 0.3923; atol = 1.0e-4), randomised_bb)
