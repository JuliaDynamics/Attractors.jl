...
@testset "Stagger Tests" begin
    ...
    xi = Attractors.stagger_trajectory(df, x0, 30, isinside, :exp, 2.0, 1.1, Int(1.0e5), 10000, Xoshiro(123))
    ...
    for m in [:exp, :unif, :adaptive]
        v = stagger_and_step(df, xi, 10, isinside; δ₀ = 1.0e-3, stagger_mode = m)
        for u in v
    ...
    sampler, isinside = statespace_sampler(HRectangle(R_min, R_max))
    xi = Attractors.stagger_trajectory(ds, sampler(), 30, isinside, :exp, 2.0, 1.1, Int(1.0e5), 10000, Xoshiro(123))

    @test isinside(xi)
    @test Attractors.escape_time!(ds, xi, isinside, 10000) > 30

    v = stagger_and_step(ds, xi, 1000, isinside; stagger_mode = :adaptive, δ = 1.0e-4, Tm = 10, max_steps = Int(1.0e5), δ₀ = 1.0, rng = Xoshiro(123))
