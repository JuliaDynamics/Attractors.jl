...
function test_basins(
        ds, u0s, grid, expected_fs_raw, featurizer;
        rerr = 1.0e-3, ferr = 1.0e-3, aerr = 1.0e-15, ε = nothing, max_distance = Inf,
    ...
    @testset "Featurizing, pairwise comparison" begin
        config = GroupViaPairwiseComparison(;
...
@testset "Analytic dummy map" begin
    ...
    test_basins(
        ds, u0s, grid, expected_fs_raw, featurizer;
        max_distance = 20, ε = 1.0e-1, proximity_test = false, threshold_pairwise = 1,
        rerr = 1.0e-1, ferr = 1.0e-1, aerr = 1.0e-15
    )
end
...
@testset "Henon map: discrete & divergence" begin
    ...
    test_basins(
        ds, u0s, grid, expected_fs_raw, featurizer;
        max_distance = 20, ε = 1.0e-3, proximity_test = false, threshold_pairwise = 1
    )
end
...
@testset "Magnetic pendulum: projected system" begin
    ...
    function magnetic_pendulum(
            u = [sincos(0.12553 * 2π)..., 0, 0];
    ...
    diffeq = (alg = Vern9(), reltol = 1.0e-9, abstol = 1.0e-9)
    ...
    test_basins(ds, u0s, grid, expected_fs_raw, featurizer; ε = 0.2, Δt = 1.0, ferr = 1.0e-2, featurizer_matrix, pairwise_comparison_matrix_test = true, threshold_pairwise = 1)
end
...
if DO_EXTENSIVE_TESTS
    ...
    @testset "Lorenz-84 system: interlaced close-by" begin # warning, super expensive test
        ...
        test_basins(
            ds, u0s, grid, expected_fs_raw, featurizer;
            ε = 0.01, ferr = 1.0e-2, Δt = 0.2, consecutive_attractor_steps = 5, Ttr = 100, threshold_pairwise = 100
        ) #threshold is very high because features haven't really converged yet here
    end
    ...
    @testset "Duffing oscillator: stroboscopic map" begin
        ...
        test_basins(
            ds, u0s, grid, expected_fs_raw, featurizer;
            ε = 1.0, ferr = 1.0e-2, rerr = 1.0e-2, aerr = 5.0e-3, threshold_pairwise = 1
        )
    end
    ...
    @testset "Thomas cyclical: Poincaré map" begin
        ...
        ds = CoupledODEs(thomas_rule, [1.0, 0, 0], [0.1665]; diffeq = (reltol = 1.0e-9,))
        ...
        pmap = PoincareMap(
            ds, (3, 0.0);
            Tmax = 1.0e6,
            rootkw = (xrtol = 1.0e-8, atol = 1.0e-8),
        )
        u0s = [
            1 => [1.83899, -4.15575, 0],
        ...
        test_basins(pmap, u0s, grid, expected_fs_raw, thomas_featurizer; ε = 1.0, ferr = 1.0e-2)
    end
